"""
IC-pi Platform: Seed DANA Case Study Data
Run AFTER the app is deployed: python seed_dana.py
"""
...
for name, critical, source, rf, gt, bw in param_data:
    ...
    db.flush()
    params.append(p)
    print(f"  Parameter: {p.name} ({'CRITICAL' if critical else 'normal'})")

# KPIs
kpi_data = [
    (params[0], "Documented protocol exists", "binary", 0.00),
    (params[0], "Protocol tested in drill (last 12mo)", "binary", 0.00),
